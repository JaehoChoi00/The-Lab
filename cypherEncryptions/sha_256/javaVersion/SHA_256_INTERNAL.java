...
import static shared.javaUtil.VariableConstants.FG_CYAN;
...
public class SHA_256_INTERNAL {
    ...
    public String run(String textToHash) {
        System.out.printf(BOLD + UNDERLINE +"%c[SHA-256]" + BOLD + UNDERLINE +  RESET, (char) LINEFEED);
        NEWLINE();

        System.out.printf("Input: " + BOLD + FG_GREEN + "%s\n" + RESET, textToHash);
        System.out.printf("Input length: " + BOLD + FG_GREEN + "%d\n" + RESET, textToHash.length());

        int[] message = convertToByteUTF8(textToHash);
        long bitLength = (long) message.length * 8;

        System.out.printf("bit length: " + BOLD + FG_GREEN + "%d\n" + RESET, bitLength);
        System.out.printf("Input converted to bytes: " + BOLD + FG_GREEN + "\n%s" + RESET, java.util.Arrays.toString(message));
        LINEBREAK();

        // Step 1: Padding
        System.out.printf(BOLD + UNDERLINE +"%c[Step 1: Padding]" + BOLD + UNDERLINE +  RESET, (char) LINEFEED);
        NEWLINE();

        int[] paddedBinary = new int[message.length + 1];

        System.out.printf("Padded binary: Size: " + BOLD + FG_COLOR(150) + "%d" + RESET + BOLD + FG_GREEN + "\n%s" + RESET, message.length + 1, java.util.Arrays.toString(paddedBinary));
        NEWLINE();
        ...
        // System.out.println("");
        System.out.printf("Convert input to decimals: " + BOLD + FG_GREEN + "\n%s" + RESET, java.util.Arrays.toString(paddedBinary));
        NEWLINE();

        paddedBinary[message.length] = singleBitPadding;

        System.out.printf("Single byte padding added at the end: " + BOLD + FG_GREEN + "\n%s" + RESET, java.util.Arrays.toString(paddedBinary));
        NEWLINE();

        int messageLengthBytes = message.length;
        int totalBytesNeeded = ((messageLengthBytes + 1 + 8 + 63) / 64) * 64;

        System.out.printf("Calculated total bytes needed: (messageLengthBytes + 1 + 8 + 63) / 64) * 64 = " + BOLD + FG_COLOR(150) + "%d" + RESET, totalBytesNeeded);
        NEWLINE();
        ...
        System.out.printf("Bytes64 array made: Size: " + BOLD + FG_COLOR(150) + "%d" + RESET + BOLD + FG_GREEN + "\n%s" + RESET, message.length + 1, java.util.Arrays.toString(bytes64));
        NEWLINE();
        ...
        System.out.printf("Filling up the space with input: " + BOLD + FG_GREEN + "\n%s" + RESET, java.util.Arrays.toString(bytes64));
        NEWLINE();
        ...
        System.out.printf("Padding remaining space: " + BOLD + FG_GREEN + "\n%s" + RESET, java.util.Arrays.toString(bytes64));
        NEWLINE();

        String bitToBinaryLength = convertToBinary((int) bitLength, 64);
        System.out.printf("Convert Input bit length to binary: " + BOLD + FG_GREEN + "%s\n" + RESET, bitToBinaryLength);
        ...
        for (index = 0; index < 8; index++) {
            ...
            System.out.printf(BOLD + FG_COLOR(150) + "Converting 8bit index" + BOLD + FG_CYAN + "[%d]" + RESET + ": " + BOLD + FG_GREEN + "%s" + RESET + " to variable: " + BOLD + FG_COLOR(150) + "%d\n" + RESET, index, eightBit, bytes64[endOfBytes64 + index]);
        }
        NEWLINE();

        System.out.printf("Imprinting byte size of input: " + BOLD + FG_GREEN + "\n%s" + RESET, java.util.Arrays.toString(bytes64));
        NEWLINE();
        ...
        System.out.printf(BOLD + UNDERLINE +"%c[Step 2: 32 bits]" + BOLD + UNDERLINE +  RESET, (char) LINEFEED);
        NEWLINE();

        // 32 bits
        int totalWordsCount = bytes64.length / 4;

        System.out.printf("total word count: " + BOLD + FG_GREEN + "%d" + RESET, totalWordsCount);
        NEWLINE();

        String[] bits32 = new String[totalWordsCount];
        Arrays.fill(bits32, "");
        
        System.out.printf("Empty bits32 array made: " + BOLD + FG_GREEN + "\n%s" + RESET, java.util.Arrays.toString(bits32));
        NEWLINE();
        ...
        System.out.printf("Converting bytes64 to binary bits32: " + BOLD + FG_GREEN + "\n%s" + RESET, java.util.Arrays.toString(bits32));
        NEWLINE();
        ...
        System.out.printf("Converting Hash CupA hex: " + BOLD + FG_GREEN + "%x" + RESET + " to binary: " + BOLD + FG_COLOR(150) + "%s\n" + RESET, H[0], cupA);
        System.out.printf("Converting Hash CupB hex: " + BOLD + FG_GREEN + "%x" + RESET + " to binary: " + BOLD + FG_COLOR(150) + "%s\n" + RESET, H[1], cupB);
        System.out.printf("Converting Hash CupC hex: " + BOLD + FG_GREEN + "%x" + RESET + " to binary: " + BOLD + FG_COLOR(150) + "%s\n" + RESET, H[2], cupC);
        System.out.printf("Converting Hash CupD hex: " + BOLD + FG_GREEN + "%x" + RESET + " to binary: " + BOLD + FG_COLOR(150) + "%s\n" + RESET, H[3], cupD);
        System.out.printf("Converting Hash CupE hex: " + BOLD + FG_GREEN + "%x" + RESET + " to binary: " + BOLD + FG_COLOR(150) + "%s\n" + RESET, H[4], cupE);
        System.out.printf("Converting Hash CupF hex: " + BOLD + FG_GREEN + "%x" + RESET + " to binary: " + BOLD + FG_COLOR(150) + "%s\n" + RESET, H[5], cupF);
        System.out.printf("Converting Hash CupG hex: " + BOLD + FG_GREEN + "%x" + RESET + " to binary: " + BOLD + FG_COLOR(150) + "%s\n" + RESET, H[6], cupG);
        System.out.printf("Converting Hash CupH hex: " + BOLD + FG_GREEN + "%x" + RESET + " to binary: " + BOLD + FG_COLOR(150) + "%s" + RESET, H[7], cupH);
        NEWLINE();

        for (int blockOffset = 0; blockOffset < totalWordsCount; blockOffset += 16) {
            System.out.printf(BOLD + "Block " + RESET + BOLD + FG_COLOR(150) + "[%d]:" + RESET, blockOffset);
            NEWLINE();
            ...
            System.out.printf("Filling first 16 with bits32: " + BOLD + FG_GREEN + "\n%s" + RESET, java.util.Arrays.toString(words64));
            ...
            for (index = 16; index < 64; index++) {
                ...
                System.out.printf(BOLD + FG_COLOR(150) + "words64 index" + BOLD + FG_CYAN + "[%d]" + RESET + ": " + BOLD + FG_GREEN + "%s\n" + RESET, index, words64[index]);
                // System.out.printf(BOLD + FG_COLOR(150) + "words64 look back " + BOLD + FG_CYAN + "[%d]" + RESET + ": " + BOLD + FG_GREEN + "%s\n" + RESET, index - 2, wordMinus2);
                // System.out.printf(BOLD + FG_COLOR(150) + "words64 look back " + BOLD + FG_CYAN + "[%d]" + RESET + ": " + BOLD + FG_GREEN + "%s\n" + RESET, index - 7, wordMinus7);
                // System.out.printf(BOLD + FG_COLOR(150) + "words64 look back " + BOLD + FG_CYAN + "[%d]" + RESET + ": " + BOLD + FG_GREEN + "%s\n" + RESET, index - 15, wordMinus15);
                // System.out.printf(BOLD + FG_COLOR(150) + "words64 look back " + BOLD + FG_CYAN + "[%d]" + RESET + ": " + BOLD + FG_GREEN + "%s\n" + RESET, index - 16, wordMinus16);
                // System.out.printf(BOLD + FG_COLOR(150) + "small sigma0: " + BOLD + FG_GREEN + "%s\n" + RESET, s0);
                // System.out.printf(BOLD + FG_COLOR(150) + "small sigma1: " + BOLD + FG_GREEN + "%s\n" + RESET, s1);
                System.out.printf(BOLD + FG_COLOR(150) + "binaryAddition Result: " + BOLD + FG_COLOR(13) + "%s" + RESET, words64[index]);
                LINEBREAK();
                // System.out.println("wordMinus16: " + wordMinus16 + " s0: " + s0 + " wordMinus7: " + wordMinus7 + " s1: " + s1 + " = " + words64[index]);
            }
            NEWLINE();

            System.out.printf("Using small sigma functions to stretch after 16: " + BOLD + FG_GREEN + "\n%s" + RESET, java.util.Arrays.toString(words64));
            NEWLINE();

            // Stage 4: The 64-Round Compression Loop
            System.out.printf(BOLD + UNDERLINE +"%c[Step 3: The 64-Round Compression Loop]" + BOLD + UNDERLINE +  RESET, (char) LINEFEED);
            NEWLINE();
            ...
            for (int loop = 0; loop < 64; loop++) {
                ...
                System.out.printf(BOLD + FG_COLOR(150) + "\nwords64 index" + BOLD + FG_CYAN + "[%d]" + RESET + ": " + BOLD + FG_GREEN + "%s\n" + RESET, loop, words64[loop]);
                System.out.printf(BOLD + FG_COLOR(150) + "bigSigma filter 0: " +  BOLD + FG_GREEN + "%s\n" + RESET, filter0);
                System.out.printf(BOLD + FG_COLOR(150) + "bigSigma filter 1: " +  BOLD + FG_GREEN + "%s\n" + RESET, filter1);
                System.out.printf(BOLD + FG_COLOR(150) + "choose filter " + BOLD + FG_CYAN + "[%s]\n" + RESET, chooseFilter);
                System.out.printf(BOLD + FG_COLOR(150) + "choose filter " + BOLD + FG_CYAN + "[%s]\n" + RESET, majorityFilter);
                System.out.printf(BOLD + FG_COLOR(150) + "Temp1 -> words64[loop] + convertToBinary(K[loop], 32) + chooseFilter + bigSigma filter1 + cupH = " + BOLD + FG_CYAN + "\n[%s]\n" + RESET, temp1);
                System.out.printf(BOLD + FG_COLOR(150) + "Temp2 -> filter0 + majorityFilter = " + BOLD + FG_CYAN + "[%s]" + RESET, temp2);
                NEWLINE();
                // System.out.printf("old CupA: " + BOLD + FG_GREEN + "%s" + RESET + " new CupA: " + BOLD + FG_COLOR(150) + "%s\n" + RESET, copyA, cupA);
                // System.out.printf("old CupB: " + BOLD + FG_GREEN + "%s" + RESET + " new CupB: " + BOLD + FG_COLOR(150) + "%s\n" + RESET, copyB, cupB);
                // System.out.printf("old CupC: " + BOLD + FG_GREEN + "%s" + RESET + " new CupC: " + BOLD + FG_COLOR(150) + "%s\n" + RESET, copyC, cupC);
                // System.out.printf("old CupD: " + BOLD + FG_GREEN + "%s" + RESET + " new CupD: " + BOLD + FG_COLOR(150) + "%s\n" + RESET, copyD, cupD);
                // System.out.printf("old CupE: " + BOLD + FG_GREEN + "%s" + RESET + " new CupE: " + BOLD + FG_COLOR(150) + "%s\n" + RESET, copyE, cupE);
                // System.out.printf("old CupF: " + BOLD + FG_GREEN + "%s" + RESET + " new CupF: " + BOLD + FG_COLOR(150) + "%s\n" + RESET, copyF, cupF);
                // System.out.printf("old CupG: " + BOLD + FG_GREEN + "%s" + RESET + " new CupG: " + BOLD + FG_COLOR(150) + "%s\n" + RESET, copyG, cupG);
                // System.out.printf("old CupH: " + BOLD + FG_GREEN + "%s" + RESET + " new CupH: " + BOLD + FG_COLOR(150) + "%s" + RESET, copyH, cupH);
                // LINEBREAK();
            ...
            NEWLINE();
            System.out.printf(BOLD + UNDERLINE + "[Step 4: Block State Accumulation Update]" + RESET);
            NEWLINE();
            System.out.printf("old H[0]: " + BOLD + FG_GREEN + "%s" + RESET + "  +  CupA: " + BOLD + FG_COLOR(150) + "%s\n" + RESET, oldH0, cupA);
            System.out.printf("old H[1]: " + BOLD + FG_GREEN + "%s" + RESET + "  +  CupB: " + BOLD + FG_COLOR(150) + "%s\n" + RESET, oldH1, cupB);
            System.out.printf("old H[2]: " + BOLD + FG_GREEN + "%s" + RESET + "  +  CupC: " + BOLD + FG_COLOR(150) + "%s\n" + RESET, oldH2, cupC);
            System.out.printf("old H[3]: " + BOLD + FG_GREEN + "%s" + RESET + "  +  CupD: " + BOLD + FG_COLOR(150) + "%s\n" + RESET, oldH3, cupD);
            System.out.printf("old H[4]: " + BOLD + FG_GREEN + "%s" + RESET + "  +  CupE: " + BOLD + FG_COLOR(150) + "%s\n" + RESET, oldH4, cupE);
            System.out.printf("old H[5]: " + BOLD + FG_GREEN + "%s" + RESET + "  +  CupF: " + BOLD + FG_COLOR(150) + "%s\n" + RESET, oldH5, cupF);
            System.out.printf("old H[6]: " + BOLD + FG_GREEN + "%s" + RESET + "  +  CupG: " + BOLD + FG_COLOR(150) + "%s\n" + RESET, oldH6, cupG);
            System.out.printf("old H[7]: " + BOLD + FG_GREEN + "%s" + RESET + "  +  CupH: " + BOLD + FG_COLOR(150) + "%s\n" + RESET, oldH7, cupH);
            LINEBREAK();
        ...
        System.out.printf(BOLD + UNDERLINE + "[Stage 5: Final Hexadecimal Conversion]" + RESET);
        NEWLINE();
        System.out.printf("H[0] Binary -> Hex: " + BOLD + FG_GREEN + "%s" + RESET + "  ->  " + BOLD + FG_COLOR(150) + "%s\n" + RESET, convertToBinary(H[0], 32), finalA);
        System.out.printf("H[1] Binary -> Hex: " + BOLD + FG_GREEN + "%s" + RESET + "  ->  " + BOLD + FG_COLOR(150) + "%s\n" + RESET, convertToBinary(H[1], 32), finalB);
        System.out.printf("H[2] Binary -> Hex: " + BOLD + FG_GREEN + "%s" + RESET + "  ->  " + BOLD + FG_COLOR(150) + "%s\n" + RESET, convertToBinary(H[2], 32), finalC);
        System.out.printf("H[3] Binary -> Hex: " + BOLD + FG_GREEN + "%s" + RESET + "  ->  " + BOLD + FG_COLOR(150) + "%s\n" + RESET, convertToBinary(H[3], 32), finalD);
        System.out.printf("H[4] Binary -> Hex: " + BOLD + FG_GREEN + "%s" + RESET + "  ->  " + BOLD + FG_COLOR(150) + "%s\n" + RESET, convertToBinary(H[4], 32), finalE);
        System.out.printf("H[5] Binary -> Hex: " + BOLD + FG_GREEN + "%s" + RESET + "  ->  " + BOLD + FG_COLOR(150) + "%s\n" + RESET, convertToBinary(H[5], 32), finalF);
        System.out.printf("H[6] Binary -> Hex: " + BOLD + FG_GREEN + "%s" + RESET + "  ->  " + BOLD + FG_COLOR(150) + "%s\n" + RESET, convertToBinary(H[6], 32), finalG);
        System.out.printf("H[7] Binary -> Hex: " + BOLD + FG_GREEN + "%s" + RESET + "  ->  " + BOLD + FG_COLOR(150) + "%s\n" + RESET, convertToBinary(H[7], 32), finalH);
        NEWLINE();

        String finalOutput = finalA + finalB + finalC + finalD + finalE + finalF + finalG + finalH;
        System.out.printf(BOLD + FG_GREEN + "FINAL COMBINED OUTPUT" + RESET + ": " + BOLD + FG_COLOR(150) + "%s" + RESET, finalOutput);
        LINEBREAK();
